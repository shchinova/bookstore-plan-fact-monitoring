...
| `inventory_update`      | Остатки за 15.04.2026 (для эмуляции ежедневного обновления) |

Первичная загрузка сырых данных в PostgreSQL выполняется с помощью `load_history.py`:
1) скрипт читает файлы из `data/raw/` (кроме "выгрузок" за 15.04.2026: `sales_update.csv` и `inventory_update.csv`),
...
5) создаёт витрины данных по SQL-скриптам из `sql/`.

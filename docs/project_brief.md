...
| `fact_plan.csv`         | Планы                                                       |
| `sales_history.csv`     | Продажи за 01.01.2024 — 14.04.2026                          |
| `sales_update.csv`      | Продажи за 15.04.2026 (для эмуляции ежедневного обновления) |
| `inventory_history.csv` | Остатки за 01.01.2024 — 14.04.2026                          |
| `inventory_update.csv`  | Остатки за 15.04.2026 (для эмуляции ежедневного обновления) |

### ШАГ 3. Загрузка данных в PostgreSQL

Первичная загрузка сырых данных в PostgreSQL выполняется с помощью `load_history.py`:
1) скрипт создаёт таблицы базы данных через `schema.sql`,
2) читает все файлы из `data/raw/`, кроме `sales_update.csv` и `inventory_update.csv`,
3) выполняет проверку и очистку данных через `validate.py`,
4) сохраняет отчёт об аномалиях в `reports/`,
5) загружает очищенные данные в PostgreSQL,
6) создаёт витрины данных по SQL-скриптам через `refresh_all_marts.sql`.

После загрузки база содержит данные по **14.04.2026 включительно**.

**Эмуляция ежедневного обновления** данных (в демо это обновление на 15.04.2026) выполняется с помощью `daily_update.py`:
1) скрипт читает `sales_update.csv` и `inventory_update.csv` из `data/raw/`,
2) выполняет проверку и очистку через `validate.py`,
...
### ШАГ 5. Создание дашборда в Tableau Public
...
### ШАГ 6. Настройка автоматических уведомлений
...
### ШАГ 7. Проведение аналитических исследований

...
## Методология создания базы данных

**В production** создание базы данных проходит следующие этапы:
...
**В демо** процесс упрощён для локального воспроизведения (см. раздел «Ход работы в демо») по ряду причин:
- В проде нужны дополнительные пользователи, миграции, автоматизация по расписанию.
- В демо достаточно минимальной работающей версии для демонстрации логики.

## Интеграция с 1С и обновление базы данных
...
**В production** обновление базы данных возможно в двух вариантах:
...
**В production** PostgreSQL был бы установлен на сервер (Linux/Windows) с созданием отдельных пользователей БД и настройкой прав доступа. **В демо** процесс упрощён до минимальной работающей версии для демонстрации логики.

| Шаг | Действие |
|-----|----------|
| **1** | PostgreSQL установлен на локальный ПК (порт 5432, пароль задан при установке) |
| **2** | База данных создана вручную одной командой: `CREATE DATABASE portfolio_project;` |
...
### ШАГ 4. Создание дашборда в Tableau Public
...
### ШАГ 5. Настройка автоматических уведомлений
...
### ШАГ 6. Проведение аналитических исследований

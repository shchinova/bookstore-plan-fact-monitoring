...
В `mart_daily_pulse.sql` витрина разбита на три таблицы для удобства подключения в Power Query:
...
**В production** скрипт `alerts.py` запускается ежедневно по расписанию (Windows Task Scheduler или cron) сразу после `daily_update.py`.
1. Скрипт обращается к витрине `mart_inventory_alerts`.
...
3. Каждая отправка фиксируется в `reports/alerts_log.jsonl`; файл только дополняется, история не перезаписывается.
...
**В демо** для демонстрации ежедневный запуск по расписанию не настраивался. Скрипт был запущен вручную два раза: после первичной загрузки данных и после обновления данных на 15.04.2026. Письма с таблицами товаров ниже порога были получены на тестовый адрес — скриншот приложен ниже. Это подтверждает, что код работает корректно, SMTP-соединение устанавливается, HTML-шаблон рендерится правильно, фильтрация по ролям получателей функционирует.

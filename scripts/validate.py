...
История исправлений
───────────────────
[FIX-1] _log принимает параметр n: счётчики растут на реальное кол-во строк,
        а не на 1 при каждом вызове.

[FIX-2] Дата записывается обратно в df строкой 'YYYY-MM-DD' (strftime) после
        парсинга, а не Python date-объектом: колонки со StringDtype не
        принимают не-строковые значения.

[FIX-3] validate_fact_sales S-07: clean['date'] явно приводится к datetime
        перед pd.date_range.

[FIX-4] validate_dim_product P-16: уровень 'warn' → 'reject', строки реально
        удаляются, счётчик rows_rejected должен это отражать.

[FIX-5] validate_fact_sales S-19: добавлена проверка lost_sales_qty >= 0.

[FIX-6] validate_fact_inventory I-05: счётчик _log передаёт n=кол-во дублей.

[FIX-7] _apply_general_rules: StringDtype → object в самом начале. При
        read_csv(dtype=str) pandas использует StringDtype, в которую нельзя
        присваивать числа или флаги — конвертация в object снимает это
        ограничение для всего последующего кода.

[FIX-8] _to_int_flag: универсальная конвертация флаговых колонок из любого
        строкового/булева представления ('True'/'False', '1'/'0', True/False)
        в целочисленный 0/1.

[FIX-9] _coerce_numeric: централизованная ранняя конвертация числовых колонок
        в начале каждой функции validate — до любых сравнений и проверок.
        Устраняет TypeError при операциях вида clean['qty'] > 0, когда
        колонка ещё остаётся строкой object после read_csv(dtype=str).

[FIX-10] validate_dim_date D-04: year/month/quarter сравниваются с числами
         только после _coerce_numeric, иначе '2024' != 2024.

[FIX-11] Флаговые колонки (is_physical, is_weekend, is_low_stock) всегда
         перезаписываются вычисленным значением 0/1, а не только при
         расхождении — это гарантирует числовой тип в выходном датафрейме.
"""
...
# Жанры для fact_plan (без Subscription)
GENRES_PLAN = {
    'Fiction', 'Fantasy', 'Mystery', 'Romance', 'Science',
...
def _log(report: dict, level: str, code: str, message: str, n: int = 1) -> None:
    """
    Добавляет запись в отчёт и увеличивает счётчик на n строк.
    [FIX-1] level: 'reject' | 'fix' | 'warn'
    """
...
def _to_int_flag(series: pd.Series) -> pd.Series:
    """
    [FIX-8] Приводит флаговую колонку к 0/1, обрабатывая все форматы
    которые может породить read_csv(dtype=str):
      '1'/'0', 'True'/'False', 'true'/'false', True/False, 1/0.
...
def _coerce_numeric(df: pd.DataFrame, cols: list[str]) -> pd.DataFrame:
    """
    [FIX-9] Конвертирует указанные колонки в числовой тип (float64) через
    pd.to_numeric с errors='coerce'. Вызывается в начале каждой функции
    validate, до любых арифметических сравнений — иначе строки из
    read_csv(dtype=str) вызывают TypeError: '>' not supported between str and int.

    Колонки, отсутствующие в df, пропускаются без ошибки.
    """
    for col in cols:
...
def _apply_general_rules(df: pd.DataFrame, report: dict) -> pd.DataFrame:
    ...
    """
    # [FIX-7] StringDtype → object: pandas read_csv(dtype=str) возвращает
    # колонки с типом StringDtype, в которые нельзя присваивать числа.
    # Конвертируем всё в object — стандартный тип для смешанных данных.
    ...
    # G-04: убираем пробелы по краям
    fixed_strip = 0
    for col in str_cols:
        mask = df[col].notna() & (df[col].str.strip() != df[col])
        if mask.any():
            df.loc[mask, col] = df.loc[mask, col].str.strip()
            fixed_strip += int(mask.sum())
    if fixed_strip:
        _log(report, 'fix', 'G-04',
             f'Лишние пробелы обрезаны: {fixed_strip} значений', n=fixed_strip)

    # G-03: пустые строки и строки из пробелов → NaN
    fixed_empty = 0
    for col in str_cols:
        mask = df[col].notna() & (df[col].str.strip() == '')
        if mask.any():
            df.loc[mask, col] = np.nan
            fixed_empty += int(mask.sum())
    if fixed_empty:
        _log(report, 'fix', 'G-03',
             f'Пустые строки заменены на NULL: {fixed_empty} значений', n=fixed_empty)
...
def _try_parse_date(series: pd.Series) -> pd.Series:
    """Парсит серию в datetime; нераспознанные значения → NaT."""
...
def validate_dim_date(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    # [FIX-9] Ранняя конвертация числовых колонок
    df = _coerce_numeric(df, ['year', 'month', 'quarter', 'is_weekend', 'is_holiday_ru'])
    ...
    # [FIX-2] strftime → строка, совместима с StringDtype и object
    df.loc[~bad_date, 'date'] = parsed[~bad_date].dt.strftime('%Y-%m-%d')
    ...
    # D-04: year, month, quarter пересчитываем из даты
    # [FIX-10] После _coerce_numeric колонки уже float/int — сравнение корректно
    expected_year    = parsed_clean.dt.year
    expected_month   = parsed_clean.dt.month
    expected_quarter = parsed_clean.dt.quarter
    mismatches = (
        (clean['year'].fillna(-1).astype(int)    != expected_year)   |
        (clean['month'].fillna(-1).astype(int)   != expected_month)  |
        (clean['quarter'].fillna(-1).astype(int) != expected_quarter)
    )
    if mismatches.any():
    ...
    # D-05: is_weekend — всегда перезаписываем вычисленным значением
    # [FIX-11] Гарантируем 0/1 в выходном df независимо от входного формата
    expected_weekend = (parsed_clean.dt.weekday >= 5).astype(int)
    bad_weekend = _to_int_flag(clean['is_weekend']) != expected_weekend
    if bad_weekend.any():
        n = int(bad_weekend.sum())
        _log(report, 'fix', 'D-05', f'is_weekend пересчитан: {n} строк', n=n)
    clean['is_weekend'] = expected_weekend

    # D-06: is_holiday_ru — должен быть 0 или 1
    bad_holiday = ~_to_int_flag(clean['is_holiday_ru']).isin([0, 1])
    if bad_holiday.any():
        n = int(bad_holiday.sum())
        clean = clean[~bad_holiday]
        _log(report, 'reject', 'D-06',
             f'Недопустимое значение is_holiday_ru: {n} строк', n=n)
...
def validate_dim_product(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    df = _coerce_numeric(df, [
        'product_id', 'price_rub', 'cost_rub',
        'avg_rating', 'review_count', 'stock_initial', 'page_count',
    ])
    ...
    if bad_id.any():
        n = int(bad_id.sum())
        reject_mask |= bad_id
        _log(report, 'reject', 'P-01', f'product_id IS NULL или <= 0: {n} строк', n=n)
    ...
    warn_page = clean['format'].isin(PHYSICAL_FORMATS) & clean['page_count'].isna()
    if warn_page.any():
        n = int(warn_page.sum())
        _log(report, 'warn', 'P-11',
             f'page_count IS NULL у Paperback/Hardcover: {n} строк', n=n)
    ...
    if fix_stock.any():
        ...
        _log(report, 'fix', 'P-12',
             f'stock_initial обнулён у цифровых форматов: {n} строк', n=n)

    # P-15: is_physical всегда перезаписываем из format
    # [FIX-11] Гарантируем 0/1 независимо от входного формата ('True'/'False' и т.п.)
    expected_physical = clean['format'].isin(PHYSICAL_FORMATS).astype(int)
    bad_physical = _to_int_flag(clean['is_physical']) != expected_physical
    if bad_physical.any():
        n = int(bad_physical.sum())
        _log(report, 'fix', 'P-15', f'is_physical пересчитан из format: {n} строк', n=n)
    clean['is_physical'] = expected_physical

    # P-16: полные дубликаты записей (все поля кроме product_id)
    # [FIX-4] 'reject' вместо 'warn': строки реально удаляются из датафрейма
    ...
    if dupes_rec.any():
        ...
        _log(report, 'reject', 'P-16',
             f'Полные дубликаты записей (кроме product_id): {n} строк удалено', n=n)
...
def validate_fact_plan(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    # [FIX-9] Ранняя конвертация числовых колонок
    df = _coerce_numeric(df, [
        'plan_id', 'year', 'month', 'plan_qty', 'plan_amount', 'plan_margin_target',
    ])
    ...
    # PL-02: genre (только книжные жанры, без Subscription)
    ...
    # PL-09: plan_margin_target в [0, 1]
    bad_margin = (
    ...
    if bad_margin.any():
        ...
        _log(report, 'reject', 'PL-09',
             f'plan_margin_target вне [0, 1]: {n} строк', n=n)

    clean = df[~reject_mask].copy()

    # PL-08: qty=0 при amount!=0 или наоборот
    inconsistent = (
    ...
    if inconsistent.any():
        ...
        _log(report, 'warn', 'PL-08',
             f'plan_qty/plan_amount несогласованы: {n} строк', n=n)
    ...
    if dupes_key.any():
        ...
        _log(report, 'reject', 'PL-10',
             f'Дубликаты (genre, format, year, month): {n} строк', n=n)
...
def validate_fact_sales(
    ...
    # [FIX-9] Ранняя конвертация всех числовых колонок — до любых сравнений
    df = _coerce_numeric(df, [
        'sales_id', 'order_id', 'product_id',
        'sales_qty', 'return_qty', 'unit_price',
        'sales_amount', 'return_amount',
        'discount_percent', 'is_promo', 'lost_sales_qty',
    ])
    ...
    # [FIX-2] Нормализованная дата → строка 'YYYY-MM-DD'
    valid_date_mask = ~bad_date_fmt & ~reject_mask
    df.loc[valid_date_mask, 'date'] = (
        parsed_date[valid_date_mask].dt.strftime('%Y-%m-%d')
    )
    ...
    if bad_disc.any():
        ...
        _log(report, 'reject', 'S-14',
             f'discount_percent вне [0, 100]: {n} строк', n=n)
    ...
    if dupes_op.any():
        ...
        _log(report, 'reject', 'S-03',
             f'Дубликаты (order_id, product_id): {n} строк', n=n)

    # S-07: потерянные дни
    # [FIX-3] Явное приведение к datetime перед pd.date_range
    ...
    if clean_dates_dt.notna().any():
        ...
        if missing_days:
            ...
            _log(report, 'warn', 'S-07',
                 f'Потерянные дни: {sample}{suffix}', n=len(missing_days))
    ...
    if bad_amount.any():
        n = int(bad_amount.sum())
        _log(report, 'warn', 'S-10',
             f'sales_amount расходится с sales_qty × unit_price: {n} строк', n=n)
    ...
    if bad_return_digital.any():
        ...
        _log(report, 'warn', 'S-11',
             f'return_qty > 0 у цифровых форматов: {n} строк', n=n)
    ...
    # S-13: return_amount ≈ return_qty × unit_price
    ...
    if bad_ret_amount.any():
        ...
        _log(report, 'warn', 'S-13',
             f'return_amount не соответствует return_qty × unit_price: {n} строк', n=n)
    ...
    if bad_promo.any():
        n = int(bad_promo.sum())
        _log(report, 'warn', 'S-15',
             f'is_promo=1 без скидки или promo_code: {n} строк', n=n)
    ...
    if bad_no_promo.any():
        ...
        _log(report, 'warn', 'S-16',
             f'is_promo=0 но discount_percent > 0: {n} строк', n=n)
    ...
    if bad_lost_digital.any():
        ...
        _log(report, 'warn', 'S-18',
             f'lost_sales_qty > 0 у цифровых форматов: {n} строк', n=n)

    # S-19: lost_sales_qty >= 0
    # [FIX-5] Проверка неотрицательности поля
...
def validate_fact_inventory(
    ...
    # [FIX-9] Ранняя конвертация всех числовых колонок
    df = _coerce_numeric(df, [
        'inventory_id', 'product_id',
        'opening_stock', 'sold_qty', 'replenishment_qty',
        'closing_stock', 'is_low_stock',
    ])
    ...
    if not_physical.any():
        ...
        _log(report, 'reject', 'I-02',
             f'product_id не является физическим товаром: {n} строк', n=n)
    ...
    bad_replen = (
        df['replenishment_qty'].isna() | (df['replenishment_qty'] < 0)
    ) & ~reject_mask
    if bad_replen.any():
    ...
    # [FIX-6] n= передаёт реальное количество дублей
    dupes = clean.duplicated(subset=['product_id', 'date'], keep='first')
    if dupes.any():
        n = int(dupes.sum())
        clean = clean[~dupes]
        _log(report, 'reject', 'I-05',
             f'Дубликаты (product_id, date): {n} строк', n=n)

    # I-10: баланс closing = opening - sold + replenishment
    expected_close = (
        clean['opening_stock'] - clean['sold_qty'] + clean['replenishment_qty']
    )
    ...
    if bad_balance.any():
        ...
        _log(report, 'warn', 'I-10',
             f'closing_stock ≠ opening − sold + replenishment: {n} строк', n=n)

    # I-11: is_low_stock всегда пересчитываем из closing_stock
    # [FIX-11] Гарантируем 0/1 независимо от входного формата
    expected_low = (clean['closing_stock'] < LOW_STOCK_THRESHOLD).astype(int)
    bad_low = _to_int_flag(clean['is_low_stock']) != expected_low
    if bad_low.any():
        n = int(bad_low.sum())
        _log(report, 'fix', 'I-11', f'is_low_stock пересчитан: {n} строк', n=n)
    clean['is_low_stock'] = expected_low
...
def build_report_md(reports: list[dict], script_name: str) -> str:
